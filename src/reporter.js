...
class DocxEvid extends _reporter.default {
    ...
    onTestEnd(theTest) {
        ...
        let filepath = path.join(this.options.outputDir, this.options.timestamp, this.specName, (this.ctNum + '_' + this.ctName).replace(/[\\/|<>*:"?]+/g, ''));
    ...
    onAfterCommand(command) {
        if (this.isScreenshotCommand(command) && command.result.value) {
            const filepath = path.join(this.options.outputDir, this.options.timestamp, this.specName, (this.ctNum + '_' + this.ctName).replace(/[\\/|<>*:"?]+/g, ''), '/screenshots/', this.imgNum + '.png');
            fs.outputFileSync(filepath, Buffer.from(command.result.value, 'base64'));
            this.imagesTest.push({ path: filepath });
            this.imgNum += 1;
        }
    }
...
async function buildReport(evidencePath, nameDoc, template, ctName, timeDuration, state, images) {
    ...
    let rep = createReport({
        output: evidencePath + '/' + nameDoc + '.docx',
        template: template,
        data: {
            project: {
                "name": ctName,
